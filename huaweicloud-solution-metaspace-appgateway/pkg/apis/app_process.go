...
type RegisterAppProcessRequest struct {
	...
	LaunchPath                              string `json:"launch_path" validate:"required,min=1,max=262144"`
	Parameters                              string `json:"parameters" validate:"omitempty,min=0,max=262144"`
}
...
type UpdateAppProcessRequest struct {
	...
	LogPath    []string `json:"log_path" validate:"omitempty,gte=0,lte=100"`
...
type ListMonitorAppProcessResponse struct {
	TotalCount			int					`json:"total_count"`
	Count				int 				`json:"count"`
	Processes			[]MonitorAppProcessResponse	`json:"app_processes"`
}


type MonitorAppProcessResponse struct {
...
type QueryMonitorAppProcess struct {
	ProcessID 			string
	InstanceID			string
	IpAddress			string
	FleetID				string
	State				string
	GtServerSessionNum	int
	LtServerSessionNum	int			
}
